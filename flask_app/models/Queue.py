class Queue(object):
    ...
    # );
    def __init__(self, id, lat, lng, created_at, catName, catDesc, decision, username, awsuuid):
        self.id = id
        self.lat = lat
        self.lng = lng
    ...
    def map_to_queue(rows):
        return [Queue(id=row[0], lat=row[1], lng=row[2], created_at=row[3], catName=row[4], catDesc=row[5], decision=row[6], username=row[7], awsuuid=row[8]).to_dict() for row in rows]
